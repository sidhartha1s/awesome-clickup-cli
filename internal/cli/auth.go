...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Clickup Reference",
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  github.com/sidhartha1s/awesome-clickup-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export CLICKUP_REFERENCE_AUTHORIZATION_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  github.com/sidhartha1s/awesome-clickup-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  github.com/sidhartha1s/awesome-clickup-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  github.com/sidhartha1s/awesome-clickup-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("CLICKUP_REFERENCE_AUTHORIZATION_TOKEN") != "" {
				envStillSet = "CLICKUP_REFERENCE_AUTHORIZATION_TOKEN"
			}
