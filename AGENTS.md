...
This directory is a generated `clickup-reference-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
clickup-reference-pp-cli doctor --json
clickup-reference-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
clickup-reference-pp-cli which "<capability>" --json
clickup-reference-pp-cli <command> --help
```
...
```bash
clickup-reference-pp-cli <command> --agent
```
...
```bash
clickup-reference-pp-cli <command> --help
clickup-reference-pp-cli <command> --dry-run --agent
```
