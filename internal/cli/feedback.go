...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLICKUP_REFERENCE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLICKUP_REFERENCE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/sidhartha1s/awesome-clickup-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.github.com/sidhartha1s/awesome-clickup-cli/feedback.jsonl.
When ` + "`CLICKUP_REFERENCE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLICKUP_REFERENCE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "github.com/sidhartha1s/awesome-clickup-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  github.com/sidhartha1s/awesome-clickup-cli feedback list
  github.com/sidhartha1s/awesome-clickup-cli feedback list --limit 5
  github.com/sidhartha1s/awesome-clickup-cli feedback list --json`,
