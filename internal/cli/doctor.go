...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  github.com/sidhartha1s/awesome-clickup-cli doctor
  github.com/sidhartha1s/awesome-clickup-cli doctor --json
  github.com/sidhartha1s/awesome-clickup-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CLICKUP_REFERENCE_AUTHORIZATION_TOKEN=<your-key>"
				} else {
			...
			if os.Getenv("CLICKUP_REFERENCE_AUTHORIZATION_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CLICKUP_REFERENCE_AUTHORIZATION_TOKEN")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CLICKUP_REFERENCE_AUTHORIZATION_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "github.com/sidhartha1s/awesome-clickup-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("github.com/sidhartha1s/awesome-clickup-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'github.com/sidhartha1s/awesome-clickup-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'github.com/sidhartha1s/awesome-clickup-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'github.com/sidhartha1s/awesome-clickup-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'github.com/sidhartha1s/awesome-clickup-cli sync' to refresh."
	}
	return report
}
