...
In local mode: searches locally synced data only.`,
  ...
  github.com/sidhartha1s/awesome-clickup-cli search "error timeout"

  # Force local search only
  github.com/sidhartha1s/awesome-clickup-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  github.com/sidhartha1s/awesome-clickup-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  github.com/sidhartha1s/awesome-clickup-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'github.com/sidhartha1s/awesome-clickup-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/github.com/sidhartha1s/awesome-clickup-cli/data.db)")
