...
type Config struct {
	...
	Path           string `toml:"-"`
	ClickupReferenceAuthorizationToken string `toml:"reference_authorization_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("CLICKUP_REFERENCE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "github.com/sidhartha1s/awesome-clickup-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("CLICKUP_REFERENCE_AUTHORIZATION_TOKEN"); v != "" {
		cfg.ClickupReferenceAuthorizationToken = v
		cfg.AuthSource = "env:CLICKUP_REFERENCE_AUTHORIZATION_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.ClickupReferenceAuthorizationToken != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("CLICKUP_REFERENCE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.ClickupReferenceAuthorizationToken
	if token == "" {
		return ""
	}
	if c.ClickupReferenceAuthorizationToken == "" {
		return ""
	}
	return token
}
...
func (c *Config) SaveCredential(token string) error {
	c.AuthHeaderVal = ""
	c.AccessToken = ""
	c.ClickupReferenceAuthorizationToken = token
	return c.save()
}
